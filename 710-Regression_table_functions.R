...
univar_table <- function(uni_data, predictor_vars, response_var, odds_sigdig = 2, p_sigdig = 2, p_threshold = 0.01){
  ...
  for(var in predictor_vars){
    ...
    # get 95% conf int
    conf <- confint(Uni_mod)
    
    # loop through all terms
    for(i in 1:length(Uni_mod$coef)){
      
      if(names(Uni_mod$coef[i])=="(Intercept)"){
        ...
        t_odds <- exp(unname(Uni_mod$coef[i]))
        t_p_value <- coef(summary(Uni_mod))[i,4]
        
        # round cofficients
        t_odds <- round(t_odds, digits = odds_sigdig)
        
        # get upper and lower 95% CI
        t_odds_low <- round(exp(conf[i,1]), digits = odds_sigdig)
        t_odds_high <- round(exp(conf[i,2]), digits = odds_sigdig)
...
multivar_table <- function(multi_data, predictor_vars, response_var, odds_sigdig = 2, p_sigdig = 2, p_threshold = 0.01){
  ...
  # perform univariate analysis on each variable, substituting in name of the variable 
  multi_mod <- glm(multi_form, 
                   data = multi_data, 
                   family = "binomial")  
  
  # get 95% conf int
  conf <- confint(multi_mod)
  
  # loop through all terms
  for(i in 1:length(multi_mod$coef)){
    
    if(names(multi_mod$coef[i])=="(Intercept)"){
      # for the intercept enter place holder info  
      t_odds <-  "Ref."
      t_p_value <-  "-"
      ...
      # else extract OR and p-value
      t_odds <- exp(unname(multi_mod$coef[i]))
      t_p_value <- coef(summary(multi_mod))[i,4]
    ...
    multi_analysis <- rbind(multi_analysis, 
                            data.frame(
                              var_name = names(multi_mod$coef[i]), 
                              odds = t_odds, 
                              p_value = as.character(t_p_value)))
  }
